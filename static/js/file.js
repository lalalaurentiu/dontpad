...
function codeMirrorMergeUI(target, historic, original) {
  diff = CodeMirror.MergeView(target, {
...
let socket = new WebSocket(
  wsProtocol + window.location.host + "/ws/chat/" + chatRoom + "/"
);
...
socket.onmessage = (e) => {
  ...
  if (data.code) {
    if (!data.is_professor) {
      ...
      versions.push(
        createVersions(
          versions,
          document.querySelector(".versioning"),
          data.code,
          editor
        )
      );
  ...
  if (
    data.color &&
    data.lineStart.toString() &&
    data.lineEnd.toString() &&
...
try {
  ...
  sendCode.onclick = function () {
    ...
    socket.send(
      JSON.stringify({
        chatCode: { code: code, lineStart: lineStart },
      })
    );
  ...
  let whatsapp_image_container = document.getElementById(
  ...
  share.onclick = function () {
    ...
    imgNode.setAttribute(
      "style",
      "height: 100vh; width: 100vw;position:absolute; top:0; left:0; z-index: -1;"
    );
