...
def new_file(request, slug):
    ...
    if file:
        ...
        for extension in extensions:
            try:
                fileId = DontpadURL.objects.filter(
                    slug=file + "." + extension)[0].id
                dbfile = DontpadCode.objects.filter(
                    slug_id=fileId).values().order_by("-id")[0]
                if extension == "js":
                    jsfile = dbfile
                elif extension == "css":
                    cssFile = dbfile
...
def comment(request, slug):
    ...
    if proffesorCode:
        comments = DontpadComment.objects.filter(
            slug_id=obj, proffesorCode=proffesorCode)
    elif studentCode:
        comments = DontpadComment.objects.filter(
            slug_id=obj, userCode=studentCode)
    else:
    ...
    if request.method == "POST" and request.user.is_authenticated:
        ...
        if user.get("proffesor"):
            proffesorCode = DontpadCode.objects.filter(
                id=int(user["proffesor"])).first()
            ...
            userCode = DontpadUserCode.objects.filter(
                id=int(user["student"])).first()
            DontpadComment.objects.create(
                slug_id=obj,
                comment=json.loads(request.body)["comment"],
                line=json.loads(request.body)["line"],
...
def uploadVideo(request, slug):
    ...
    if request.method == "POST":
        ...
        if video:
            video = DontpadVideo.objects.create(
                video=video, url_id=url_id, name=name)
            return HttpResponse(status=201,
...
def viewExercise(request, slug, id):
    ...
    try:
        result = DontpadExerciseResult.objects.get(
            exercise_id=id, user_id=request.user.id)
    except:
        result = None
...
def exerciseHint(request, slug, id):
    hint = DontpadExercise.objects.filter(
        slug_id=DontpadURL.objects.filter(slug=slug)[0].id, id=id)[0].hints
    if hint:
...
def postVideoCode(request, slug):
    ...
    if request.method == "POST":
        url_id = DontpadURL.objects.filter(slug=slug)[0].id
        audio = DontpadVideoCode.objects.create(
            audio=audio, url_id=url_id, json=changes)
...
def getVideoCode(request, slug, slugVideo):
    ...
    url_id = DontpadURL.objects.filter(slug=slug)[0].id
    video = DontpadVideoCode.objects.filter(
        url_id=url_id, slug=slugVideo).first()
